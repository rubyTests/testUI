      ...
      controller: function($scope, $ionicPopup, $ionicPickerI18n, $timeout) {
        ...
        var changeViewData = function() {
          ...
          if ($scope.timeEnabled) {
            ...
            console.log($scope.minute,'test');
            $scope.second = date.getSeconds();
            $scope.meridiem = $scope.hour < 12 ? "AM" : "PM";

            $scope.bind.hour = $scope.meridiemEnabled ? ($scope.hour % 12 || 12) : $scope.hour;
            $scope.bind.minute = ($scope.minute < 10 ? "0" : "") + $scope.minute;
            console.log($scope.bind.minute,'minutesss');
            $scope.bind.second = ($scope.second < 10 ? "0" : "") + $scope.second;
